...
df = spark.read \
  .format("bigquery") \
  .load("de2024-435415.labdataset.retaildata")    # project_id.datatset.tablename. Use your project id
df.printSchema()
df.show(4)
...
bucket = "temp_de2024_ciavatti"  # use your bucket 
...
ordered_word_count.write.format('bigquery') \
  .option('table', 'de2024-435415.labdataset.wordcounts') \
  .mode("append") \
  .save()
